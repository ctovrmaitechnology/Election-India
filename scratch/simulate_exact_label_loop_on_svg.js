...
// Simulated summary which is null initially
const summary = null;
...
console.log('Simulating loop with null summary:');
try {
  pathMatches.forEach(id => {
    if (id === 'py') {
      console.log('Processing py... accessing summary...');
      const stateInfo = summary[id] || {}; // This will crash!
      return;
    }
    ...
    if (!pos) return;
    console.log(`- Created text label for: ${name} (${id})`);
  });
} catch (e) {
  console.log('CRASHED WITH ERROR:', e.message);
}
