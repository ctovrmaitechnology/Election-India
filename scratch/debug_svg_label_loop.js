...
console.log('Simulating label placement with regex paths:');
foundIds.forEach(id => {
  if (id === 'py') return;
  const name = STATE_LABEL_NAMES[id];
  if (!name) {
    console.log(`Skipping id ${id} (no name defined)`);
    return;
  }
  if (['ch', 'dl', 'dn', 'dd'].includes(id)) {
    console.log(`Skipping id ${id} (micro UT)`);
    return;
  }
  const pos = LABEL_POS[id];
  if (!pos) {
    console.log(`Skipping id ${id} (no position defined)`);
    return;
  }
  console.log(`Appended text element for: ${id} -> ${name} at (${pos.x}, ${pos.y})`);
});
